      ...
      {PROGRESS_STEPS.map((stepName, index) => {
        ...
        return (
          <div key={stepNumber} className="flex items-center gap-2">
            {/* step circle */}
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${isActive ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                }`}
            >
              {stepNumber}
            </div>
